        ...
        for (const event of events[0]) {
            try {
                ...
                transactions.push({
                    ...
                    description: `Deposited ${formatEther(event.args.amount)} tokens as collateral`,
        ...
        for (const event of events[1]) {
            try {
                ...
                transactions.push({
                    ...
                    timestamp: block?.timestamp ? block.timestamp * 1000 : Date.now(),
                    hash: event.transactionHash,
                    description: `Withdrew ${formatEther(event.args.amount)} tokens`,
                    blockNumber: event.blockNumber
                })
            } catch (err) {
        ...
        for (const event of events[2]) {
            try {
                ...
                transactions.push({
                    ...
                    timestamp: block?.timestamp ? block.timestamp * 1000 : Date.now(),
                    hash: event.transactionHash,
                    description: `Borrowed ${formatEther(event.args.amount)} ETH`,
                    blockNumber: event.blockNumber
                })
            } catch (err) {
        ...
        for (const event of events[3]) {
            try {
                ...
                transactions.push({
                    ...
                    timestamp: block?.timestamp ? block.timestamp * 1000 : Date.now(),
                    hash: event.transactionHash,
                    description: `Repaid ${formatEther(event.args.amount)} ETH`,
                    blockNumber: event.blockNumber
                })
            } catch (err) {
        ...
        for (const event of events[5]) {
            try {
                ...
                transactions.push({
                    ...
                    timestamp: block?.timestamp ? block.timestamp * 1000 : Date.now(),
                    hash: event.transactionHash,
                    description: `Position liquidated for ${formatEther(event.args.amount)} ETH`,
                    blockNumber: event.blockNumber
                })
            } catch (err) {
