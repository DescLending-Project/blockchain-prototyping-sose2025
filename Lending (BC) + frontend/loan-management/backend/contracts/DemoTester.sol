...
contract DemoTester {
    ...
    function runCompleteDemo(
        ...
        (
            bytes memory accountSeal,
            bytes memory accountJournal
        ) = generateAccountProof();
        emit ProofGenerated(msg.sender, "Account");

        // generate TradFi Proof
        (
            bytes memory tradfiSeal,
            bytes memory tradfiJournal
        ) = generateTradFiProof(creditScore);
        emit ProofGenerated(msg.sender, "TradFi");

        // generate Nesting Proof
        (
            bytes memory nestingSeal,
            bytes memory nestingJournal
        ) = generateNestingProof(creditScore);
    ...
    function generateTradFiProof(
        uint256 creditScore
    ) public view returns (bytes memory seal, bytes memory journalData) {
        require(
        ...
        MockTradFiProof memory proof = MockTradFiProof({
            ...
            accountAge: "5 years",
            paymentHistory: creditScore >= 700
                ? "Excellent"
                : creditScore >= 650
                ? "Good"
                : "Fair"
    ...
    function generateNestingProof(
        uint256 tradfiCreditScore
    ) public view returns (bytes memory seal, bytes memory journalData) {
