...
describe("StablecoinManager - Unit", function () {
    ...
    it("should set stablecoin params successfully", async function () {
        ...
        expect(await manager.stablecoinLTV(addr1.address)).to.equal(80);
        expect(await manager.stablecoinLiquidationThreshold(addr1.address)).to.equal(120);
    });
    it("should revert if LTV too high", async function () {
        await expect(manager.setStablecoinParams(addr1.address, true, 95, 120)).to.be.revertedWith("LTV too high");
    });
    it("should revert if threshold too low", async function () {
        await expect(manager.setStablecoinParams(addr1.address, true, 80, 100)).to.be.revertedWith("Threshold too low");
    });
    it("should get LTV for stablecoin and non-stablecoin", async function () {
        await manager.setStablecoinParams(addr1.address, true, 80, 120);
        expect(await manager.getLTV(addr1.address)).to.equal(80);
        // Non-stablecoin (default)
        expect(await manager.getLTV(owner.address)).to.equal(75);
    });
    it("should get liquidation threshold for stablecoin and non-stablecoin", async function () {
        await manager.setStablecoinParams(addr1.address, true, 80, 120);
        expect(await manager.getLiquidationThreshold(addr1.address)).to.equal(120);
        // Non-stablecoin (default)
        expect(await manager.getLiquidationThreshold(owner.address)).to.equal(0);
    });
    ...
    it("should return default LTV for non-stablecoin", async function () {
        ...
        await manager.waitForDeployment();
        expect(await manager.getLTV(addr1.address)).to.equal(75); // DEFAULT_VOLATILE_LTV
    });
    it("should return default liquidation threshold for non-stablecoin", async function () {
        ...
        const manager = await StablecoinManager.deploy(owner.address);
        await manager.waitForDeployment();
        expect(await manager.getLiquidationThreshold(addr1.address)).to.equal(0);
    });
    it("should return default and custom LTV for stablecoin", async function () {
        ...
        expect(await manager.getLTV(addr1.address)).to.equal(85); // DEFAULT_STABLECOIN_LTV
        await manager.setStablecoinParams(addr1.address, true, 88, 110);
        expect(await manager.getLTV(addr1.address)).to.equal(88);
    });
    it("should return default and custom liquidation threshold for stablecoin", async function () {
        ...
        expect(await manager.getLiquidationThreshold(addr1.address)).to.equal(110); // DEFAULT_STABLECOIN_LIQUIDATION_THRESHOLD
