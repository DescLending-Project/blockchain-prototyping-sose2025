...
describe("Comprehensive Contract Coverage", function() {
    ...
    beforeEach(async function () {
        ...
        // Deploy InterestRateModel with proper parameters
    ...
    describe("VotingToken - Complete Coverage", function() {
        ...
        it("should handle reputation penalties", async function () {
            ...
            expect(await votingToken.balanceOf(user2.address)).to.equal(900n);

            // Test negative penalty (increase)
            await votingToken.connect(user1).penalizeReputation(user2.address, -200);
            expect(await votingToken.balanceOf(user2.address)).to.equal(1100n);
    ...
    describe("StablecoinManager - Complete Coverage", function() {
        it("should handle stablecoin management", async function () {
            ...
            expect(await stablecoinManager.borrowThresholds(mockToken.address)).to.equal(120n);
            ...
            expect(await stablecoinManager.borrowThresholds(mockToken.address)).to.equal(130n);
        ...
        it("should handle threshold calculations", async function () {
            ...
            expect(liquidationThreshold).to.equal(150n);
            expect(borrowThreshold).to.equal(120n);
            ...
            expect(defaultLiquidation).to.equal(200n); // Default values
            expect(defaultBorrow).to.equal(150n);
        });
        ...
        it("should handle edge cases", async function () {
            ...
            ).to.be.revertedWithCustomError("Borrow threshold cannot exceed liquidation threshold");
