...
describe("Fixed Comprehensive Contract Coverage", function () {
    ...
    beforeEach(async function () {
        ...
        mockPriceFeed = await MockPriceFeed.deploy(
            ethers.utils.parseUnits("2000", 8), // $2000 - using ethers v5 syntax
            8
        );
    ...
    describe("VotingToken - Complete Coverage", function () {
        ...
        it("should handle reputation penalties", async function () {
            ...
            expect(await votingToken.balanceOf(user2.address)).to.equal(90);

            // Test negative penalty (increase)
            await votingToken.connect(user1).penalizeReputation(user2.address, -20);
            expect(await votingToken.balanceOf(user2.address)).to.equal(110);
        });
    ...
    describe("InterestRateModel - Complete Coverage", function () {
        it("should calculate utilization rates correctly", async function () {
            const totalBorrowed = ethers.utils.parseEther("50");
            const totalSupplied = ethers.utils.parseEther("100");
        ...
        it("should handle parameter updates", async function () {
            const newBaseRate = ethers.utils.parseEther("0.06"); // 6%
        ...
        it("should handle edge cases", async function () {
            ...
            const zeroResult = await interestRateModel.getCurrentRates(0, ethers.utils.parseEther("100"));
            ...
            expect(maxResult.borrowRate.gt(zeroResult.borrowRate)).to.be.true;
        });
    });
    ...
    describe("LiquidityPool - Complete Coverage", function () {
        ...
        it("should handle borrowing and repayment", async function () {
            ...
            expect((await liquidityPool.userDebt(borrower1.address)).gt(ethers.utils.parseEther("5"))).to.be.true;
    ...
    describe("LendingManager - Complete Coverage", function () {
        ...
        it("should handle admin functions", async function () {
            await lendingManager.connect(owner).setCurrentDailyRate(ethers.utils.parseEther("0.001")); // 0.1%
            expect(await lendingManager.currentDailyRate()).to.equal(ethers.utils.parseEther("0.001"));
        });
    });
