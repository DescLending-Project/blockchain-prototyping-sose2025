...
describe("LiquidityPool - Basic Tests", function () {
    ...
    describe("Collateral Management", function () {
        beforeEach(async function () {
            ...
            await glintToken.connect(user1).approve(liquidityPool.address, ethers.parseEther("1000"));
            await liquidityPool.connect(user1).depositCollateral(glintToken.address, ethers.parseEther("100"));
        });
    ...
    describe("borrow", function () {
        ...
        it("should allow borrowing with sufficient collateral", async function () {
            ...
            await glintToken.transfer(user1.address, ethers.parseEther("1000"));
            await glintToken.connect(user1).approve(liquidityPool.address, ethers.parseEther("1000"));
            await liquidityPool.connect(user1).depositCollateral(glintToken.address, ethers.parseEther("200"));
    ...
    describe("Collateralization Check", function () {
        ...
        it("should return healthy position for adequate collateral", async function () {
            ...
            const borrowAmount = maxBorrow.gt(ethers.parseEther("0.1")) ? ethers.parseEther("0.1") : maxBorrow.div(2);
    ...
    describe("repay", function () {
        ...
        it("should revert with overpayment", async function () {
            ...
            const overpayment = debt.add(ethers.parseEther("1"));
    ...
    describe("Lending Functionality", function () {
        ...
        it("should allow interest claims", async function () {
            ...
            expect(finalBalance.gt(initialBalance.sub(ethers.parseEther("0.01")))).to.be.true;
        });
    });
    ...
    describe("Risk Score & Multiplier", function () {
        it("should decrease weighted risk score and multiplier as high risk loans are repaid", async function () {
            ...
            await glintToken.connect(borrower1).approve(liquidityPool.address, ethers.parseEther("1000"));

            await deployer.sendTransaction({
                to: liquidityPool.address,
                value: ethers.parseEther("20")
            });
            await liquidityPool.connect(borrower1).depositCollateral(glintToken.address, ethers.parseEther("500"));
...
describe("Stablecoin Collateral", function () {
    it("should calculate correct max borrow amount for stablecoins", async function () {
        ...
        const maxBorrow = collateral.mul(price).mul(ltv).div(100).div(ethers.toBigInt()	.from("1000000000000000000"));
    ...
    it("should use correct liquidation threshold for stablecoins", async function () {
        ...
        const expectedThreshold = ethers.toBigInt()	.from(110);
...
describe("Stablecoin Price Feed", function () {
    ...
    it("should revert if price feed is not set", async function () {
        // Remove price feed
        await liquidityPool.setPriceFeed(usdcToken.address, ethers.ZeroAddress);
        let reverted = false;
        try {
...
describe("Stablecoin Liquidation", function () {
    beforeEach(async function () {
        ...
        const collateralValue = await liquidityPool.getTotalCollateralValue(user1.address);
        const maxBorrowByCollateral = collateralValue.mul(100).div(ethers.toBigInt()	.from(requiredRatio));
        ...
        const borrowAmount = maxBorrow.gt(ethers.parseEther("10")) ?
    ...
    it("should allow recovery from liquidation with stablecoins", async function () {
        ...
        const requiredCollateralValue = debt.mul(ethers.toBigInt()	.from(requiredRatio)).div(100);

        // Calculate current collateral value
        const currentCollateralValue = currentCollateral.mul(currentPrice).div(ethers.toBigInt()	.from("1000000000000000000"));
        ...
        const additionalValueNeeded = requiredCollateralValue.gt(currentCollateralValue) ?
            requiredCollateralValue.sub(currentCollateralValue) : ethers.toBigInt()	.from(0);

        // Convert to token amount (add 10% buffer to ensure health)
        const additionalTokensNeeded = additionalValueNeeded.gt(0) ?
            additionalValueNeeded.mul(ethers.toBigInt()	.from("1000000000000000000")).mul(110).div(currentPrice.mul(100)) :
