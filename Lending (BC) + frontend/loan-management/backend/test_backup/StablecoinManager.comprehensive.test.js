...
describe("StablecoinManager - Comprehensive Coverage", function () {
    ...
    describe("Initialization", function () {
        it("should initialize with correct parameters", async function () {
            expect(await stablecoinManager.liquidityPool()).to.equal(mockLiquidityPool.address);
            expect(await stablecoinManager.timelock()).to.equal(mockTimelock.address);
            expect(await stablecoinManager.USDC()).to.equal(mockUSDC.address);
            expect(await stablecoinManager.USDT()).to.equal(mockUSDT.address);
        });

        it("should set correct constants", async function () {
            expect(await stablecoinManager.MAX_STABLECOIN_LTV()).to.equal(9500); // 95%
            expect(await stablecoinManager.DEFAULT_STABLECOIN_LIQUIDATION_THRESHOLD()).to.equal(10500); // 105%
        });

        it("should initialize USDC and USDT as stablecoins", async function () {
            expect(await stablecoinManager.isStablecoin(mockUSDC.address)).to.be.true;
            expect(await stablecoinManager.isStablecoin(mockUSDT.address)).to.be.true;

            const usdcParams = await stablecoinManager.getStablecoinParams(mockUSDC.address);
            expect(usdcParams[0]).to.be.true; // isStable
            expect(usdcParams[1]).to.equal(9000); // LTV 90%
            expect(usdcParams[2]).to.equal(10500); // liquidation threshold 105%
        });
    });
    ...
    describe("Stablecoin Parameter Management", function () {
        it("should allow timelock to set stablecoin parameters", async function () {
            await expect(
                ...
                .withArgs(mockDAI.address, true, 8500, 11000);

            const params = await stablecoinManager.getStablecoinParams(mockDAI.address);
            expect(params[0]).to.be.true;
            expect(params[1]).to.equal(8500);
            expect(params[2]).to.equal(11000);
        });
        ...
        it("should reject parameter setting from non-timelock", async function () {
            ...
            ).to.be.revertedWith("OnlyTimelockStablecoinManager");
        });
        ...
        it("should reject threshold too low", async function () {
            ...
            ).to.be.revertedWith("ThresholdTooLow");
        });
        ...
        it("should handle disabling stablecoin", async function () {
            ...
            expect(await stablecoinManager.isStablecoin(mockUSDC.address)).to.be.false;
        });
        ...
        it("should handle minimum threshold", async function () {
            ...
            const params = await stablecoinManager.getStablecoinParams(mockDAI.address);
            expect(params[2]).to.equal(10500);
        });
    });
    ...
    describe("Parameter Retrieval", function () {
        ...
        it("should return correct parameters for configured stablecoin", async function () {
            ...
            expect(params[1]).to.equal(8500); // LTV
            expect(params[2]).to.equal(11000); // liquidation threshold
        });
        ...
        it("should return default parameters for non-stablecoin", async function () {
            ...
            expect(params[1]).to.equal(0); // LTV
            expect(params[2]).to.equal(0); // liquidation threshold
        });

        it("should return correct LTV for stablecoin", async function () {
            expect(await stablecoinManager.stablecoinLTV(mockDAI.address)).to.equal(8500);
            expect(await stablecoinManager.stablecoinLTV(mockUSDC.address)).to.equal(9000);
        });

        it("should return correct liquidation threshold", async function () {
            expect(await stablecoinManager.stablecoinLiquidationThreshold(mockDAI.address)).to.equal(11000);
            expect(await stablecoinManager.stablecoinLiquidationThreshold(mockUSDC.address)).to.equal(10500);
        });
        ...
        it("should handle multiple stablecoins", async function () {
            ...
            const daiParams = await stablecoinManager.getStablecoinParams(mockDAI.address);
            const mockParams = await stablecoinManager.getStablecoinParams(user2.address);

            expect(daiParams[1]).to.equal(8500);
            expect(mockParams[1]).to.equal(7500);
            expect(daiParams[2]).to.equal(11000);
            expect(mockParams[2]).to.equal(12000);
        });
    });
    ...
    describe("Stablecoin Status Checks", function () {
        it("should correctly identify stablecoins", async function () {
            expect(await stablecoinManager.isStablecoin(mockUSDC.address)).to.be.true;
            expect(await stablecoinManager.isStablecoin(mockUSDT.address)).to.be.true;
            expect(await stablecoinManager.isStablecoin(mockDAI.address)).to.be.false;
        ...
        it("should update stablecoin status when parameters change", async function () {
            ...
            expect(await stablecoinManager.isStablecoin(mockDAI.address)).to.be.true;
    ...
    describe("Edge Cases", function () {
        ...
        it("should handle updating existing stablecoin parameters", async function () {
            ...
            const params = await stablecoinManager.getStablecoinParams(mockUSDC.address);
            expect(params[1]).to.equal(8000);
            expect(params[2]).to.equal(11500);
        });
        ...
        it("should handle extreme valid values", async function () {
            ...
            expect(params[2]).to.equal(50000);
        });
        ...
        it("should handle contract address as token", async function () {
            ...
            expect(await stablecoinManager.isStablecoin(stablecoinManager.address)).to.be.true;
        });
    });
    ...
    describe("Events", function () {
        ...
        it("should emit event when disabling stablecoin", async function () {
            await expect(
                ...
                .withArgs(mockUSDC.address, false, 0, 10500);
        });
        ...
        it("should emit event when updating existing stablecoin", async function () {
            await expect(
                ...
                .withArgs(mockUSDC.address, true, 8800, 10800);
        });
    });
    ...
    describe("Integration Tests", function () {
        it("should work with multiple stablecoins simultaneously", async function () {
            ...
            expect(usdcParams[1]).to.equal(9000);
            expect(daiParams[1]).to.equal(8500);
        });
        ...
        it("should handle complex parameter updates", async function () {
            ...
            expect(daiParams[1]).to.equal(8000);
            expect(daiParams[2]).to.equal(11500);
            expect(usdcParams[1]).to.equal(8800);
            expect(usdcParams[2]).to.equal(10800);
        });
    });
    ...
    describe("Gas Optimization", function () {
        ...
        it("should handle multiple parameter retrievals efficiently", async function () {
            ...
            for (let i = 0; i < 10; i++) {
                await stablecoinManager.getStablecoinParams(mockDAI.address);
                await stablecoinManager.isStablecoin(mockDAI.address);
            }
        });
    });
});